...
class browser extends uploader {
    ...
    protected function act_newDir() {
        if (!$this->config['Denied']['dirs']['create'] ||
            !isset($_POST['dir']) ||
            !isset($_POST['newDir']) ||
    ...
    protected function act_renameDir() {
        if (!$this->config['Denied']['dirs']['rename'] ||
            !isset($_POST['dir']) ||
    ...
    protected function act_deleteDir() {
        if (!$this->config['Denied']['dirs']['delete'] ||
            !isset($_POST['dir']) ||
            !strlen(rtrim(rtrim(trim($_POST['dir']), "/"), "\\"))
        )
            $this->errorMsg("Unknown error.");
    ...
    protected function act_upload() {
        ...
        if (!$this->config['Denied']['files']['upload'] ||
            !isset($_POST['dir'])
        )
            $this->errorMsg("Unknown error.");
    ...
    protected function act_rename() {
        $dir = $this->postDir();
        if (!$this->config['Denied']['files']['rename'] ||
            !isset($_POST['dir']) ||
            !isset($_POST['file']) ||
            !isset($_POST['newName']) ||
            !$this->checkFilename($_POST['file']) ||
    ...
    protected function act_delete() {
        $dir = $this->postDir();
        if (!$this->config['Denied']['files']['delete'] ||
            !isset($_POST['dir']) ||
            !isset($_POST['file']) ||
            !$this->checkFilename($_POST['file']) ||
            (false === ($file = "$dir/{$_POST['file']}")) ||
    ...
    protected function act_cp_cbd() {
        $dir = $this->postDir();
        if (!$this->config['Denied']['files']['copy'] ||
            !isset($_POST['dir']) ||
    ...
    protected function act_mv_cbd() {
        $dir = $this->postDir();
        if (!$this->config['Denied']['files']['move'] ||
            !isset($_POST['dir']) ||
            !is_dir($dir) || !is_readable($dir) || !dir::isWritable($dir) ||
            !isset($_POST['files']) || !is_array($_POST['files']) ||
            !count($_POST['files'])
