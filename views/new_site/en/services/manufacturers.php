<?php require_once ROOT . '/views/new_site/en/layouts/head.php'; ?>

<div class="main">

    <?php require_once ROOT . '/views/new_site/en/layouts/header.php'; ?>

    <main class="container">


        <div class="services-head" style="background-image: url('http://generalse.com/template/site/img/About/3TJ6NCTIRT.jpg');">
            <div class="container">
                <h1>Service for manufacturers</h1>
                <h4>We provide full range of After-sale services for IT, Mobile and MDA industries</h4>
                <div class="row mt-5">
                    <div class="col-md-4 mb-3">
                        <div data-scroll data-scroll-link="section-1" class="services-head-link">
                            <div class="services-head-image-box">
                                <img class="mw-100" src="http://generalse.com/template/site/img/About/OurDirectionsicons/1market-analysis.svg" alt="">
                            </div>
                            <div>
                                End-to-end Aftersale Services
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <div data-scroll data-scroll-link="section-2" class="services-head-link">
                            <div class="services-head-image-box">
                                <img class="mw-100" src="http://generalse.com/template/site/img/About/OurDirectionsicons/improvement.svg" alt="">
                            </div>
                            <div>
                                PARTS OPERATIONS
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <div data-scroll data-scroll-link="section-3" class="services-head-link">
                            <div class="services-head-image-box">
                                <img class="mw-100" src="http://generalse.com/template/site/img/About/OurDirectionsicons/1market-analysis.svg" alt="">
                            </div>
                            <div>
                                LOGISTICS AND FULFILLMENT
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <div data-scroll data-scroll-link="section-4" class="services-head-link">
                            <div class="services-head-image-box">
                                <img class="mw-100" src="http://generalse.com/template/site/img/About/OurDirectionsicons/1market-analysis.svg" alt="">
                            </div>
                            <div>
                                REPAIR AND TECHNICAL SERVICES
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <div data-scroll data-scroll-link="section-5" class="services-head-link">
                            <div class="services-head-image-box">
                                <img class="mw-100" src="http://generalse.com/template/site/img/About/OurDirectionsicons/1market-analysis.svg" alt="">
                            </div>
                            <div>
                                HARDWARE RECYCLING
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4 mb-3">
                        <div data-scroll data-scroll-link="section-6" class="services-head-link">
                            <div class="services-head-image-box">
                                <img class="mw-100" src="http://generalse.com/template/site/img/About/OurDirectionsicons/1market-analysis.svg" alt="">
                            </div>
                            <div>
                                CONTACT CENTER SERVICES
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
        <section data-scroll-div="section-1" class="container">
            <div class="row ">
                <div class="col-md-6">
                    <img class="mw-100" src="http://generalse.com/template/site/img/About/3TJ6NCTIRT.jpg" alt="">
                </div>
                <div class="col-md-6">
                    <h2 class="text-uppercase mb-5">End-to-end Aftersale Services</h2>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores facere illum optio possimus vel. A adipisci aspernatur exercitationem. Adipisci aspernatur doloremque ducimus ea eius fugiat impedit inventore nam quae unde?</p>
                </div>
            </div>
        </section>

        <section data-scroll-div="section-2" class="container">
            <div class="row ">
                <div class="col-md-6">
                    <img class="mw-100" src="http://generalse.com/template/site/img/About/3TJ6NCTIRT.jpg" alt="">
                </div>
                <div class="col-md-6">
                    <h2 class="text-uppercase mb-5">Parts Operations</h2>

                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores facere illum optio possimus vel. A adipisci aspernatur exercitationem. Adipisci aspernatur doloremque ducimus ea eius fugiat impedit inventore nam quae unde?</p>
                </div>
            </div>
        </section>

        <section data-scroll-div="section-3" class="container">
            <div class="row ">
                <div class="col-md-6">
                    <img class="mw-100" src="http://generalse.com/template/site/img/About/3TJ6NCTIRT.jpg" alt="">
                </div>
                <div class="col-md-6">
                    <h2 class="text-uppercase mb-5">Logistics and Fulfillment</h2>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores facere illum optio possimus vel. A adipisci aspernatur exercitationem. Adipisci aspernatur doloremque ducimus ea eius fugiat impedit inventore nam quae unde?</p>
                </div>
            </div>
        </section>

        <section data-scroll-div="section-4" class="container">
            <div class="row ">
                <div class="col-md-6">
                    <img class="mw-100" src="http://generalse.com/template/site/img/About/3TJ6NCTIRT.jpg" alt="">
                </div>
                <div class="col-md-6">
                    <h2 class="text-uppercase mb-5">Repair and Technical Services</h2>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores facere illum optio possimus vel. A adipisci aspernatur exercitationem. Adipisci aspernatur doloremque ducimus ea eius fugiat impedit inventore nam quae unde?</p>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores facere illum optio possimus vel. A adipisci aspernatur exercitationem. Adipisci aspernatur doloremque ducimus ea eius fugiat impedit inventore nam quae unde?</p>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores facere illum optio possimus vel. A adipisci aspernatur exercitationem. Adipisci aspernatur doloremque ducimus ea eius fugiat impedit inventore nam quae unde?</p>
                </div>
            </div>
        </section>

        <section data-scroll-div="section-5" class="container">
            <div class="row ">
                <div class="col-md-6">
                    <img class="mw-100" src="http://generalse.com/template/site/img/About/3TJ6NCTIRT.jpg" alt="">
                </div>
                <div class="col-md-6">
                    <h2 class="text-uppercase mb-5">Hardware Recycling</h2>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores facere illum optio possimus vel. A adipisci aspernatur exercitationem. Adipisci aspernatur doloremque ducimus ea eius fugiat impedit inventore nam quae unde?</p>
                </div>
            </div>
        </section>

        <section data-scroll-div="section-6" class="container">
            <div class="row ">
                <div class="col-md-6">
                    <img class="mw-100" src="http://generalse.com/template/site/img/About/3TJ6NCTIRT.jpg" alt="">
                </div>
                <div class="col-md-6">
                    <h2 class="text-uppercase mb-5">Contact Center Services</h2>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Asperiores facere illum optio possimus vel. A adipisci aspernatur exercitationem. Adipisci aspernatur doloremque ducimus ea eius fugiat impedit inventore nam quae unde?</p>
                </div>
            </div>
        </section>


    </main>
</div>

<?php require_once ROOT . '/views/new_site/en/layouts/footer.php'; ?>



