        ...
        <div class="body-content checkout">
            <div class="row">
                <table class="umbrella-table">
                    <caption>Search result for <?= iconv('WINDOWS-1251', 'UTF-8', $search)?>
                    ...
                    <?php if (isset($listDisassembly) && is_array($listDisassembly)): ?>
                        <?php foreach ($listDisassembly as $dis): ?>
                            ...
                            <tr class="goods" data-siteid="<?=$dis['site_id']?>" data-decompile="<?=$id_gs?>">
                                <td><?=$id_gs?></td>
								<td><?= \Umbrella\components\Functions::replaceSearchResult($search, $dis['name_partner'])?></td>
                                <td><?= $dis['dev_name']?></td>
                                <td><?= \Umbrella\components\Functions::replaceSearchResult($search, $dis['part_number'])?></td>
                                <td><?= \Umbrella\components\Functions::replaceSearchResult($search, $dis['serial_number'])?></td>
                                <td><?=$dis['stockName']?></td>
